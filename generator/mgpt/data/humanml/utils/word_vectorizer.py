...
Loc_list = ('left', 'right', 'clockwise', 'counterclockwise', 'anticlockwise', 'forward', 'back', 'backward', 'backwards', 'ahead', 'sideways',
...
class WordVectorizer(object):
    def __init__(self, meta_root, prefix):
        # NOTE: upstream hardcoded 'deps/t2m/glove/glove' here instead of using
        # `meta_root`, which silently ignored DATASET.WORD_VERTILIZER_PATH. Fixed
        # so the caller-supplied path (-> ${TSD_ASSETS}/glove/ in this repo) is honored.
    ...
    def __getitem__(self, item):
        # HumanML3D-style evaluator tokens are "word/POS". Custom datasets may
        # provide plain words, so use OTHER as a safe POS fallback instead of
        # crashing the validation dataloader.
        if '/' in item:
