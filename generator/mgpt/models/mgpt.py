...
class MotionGPT(BaseModel):
        ...
        for i in range(len(texts)):
            if task == "pred":
                motion = self.vae.decode(
                    torch.cat((batch["motion"][i], outputs[i])).unsqueeze(0))
            elif task in ["t2m", "m2t", "inbetween"]:
                motion = self.vae.decode(outputs[i].unsqueeze(0))
    ...
    def val_t2m_forward(self, batch):
        ...
        deterministic_test = bool(
            getattr(self.hparams.cfg.TEST, "DETERMINISTIC", False))
        do_sample = bool(
            getattr(self.hparams.cfg.TEST, "DO_SAMPLE",
                    not deterministic_test))
    ...
    def val_m2t_forward(self, batch):
        ...
        for i in range(len(feats_ref)):
            tokens_i, lens_i = self.vae.encode(feats_ref[i:i + 1])
            motion_tokens.append(tokens_i[0])
            lengths_tokens.append(tokens_i.shape[1])
    ...
    def val_vae_forward(self, batch, split="train"):
        ...
        for i in range(len(feats_ref)):
            ...
            code_pred, _ = self.vae.encode(feats_ref[i:i + 1, :lengths[i]])  # (tokens, lengths)
