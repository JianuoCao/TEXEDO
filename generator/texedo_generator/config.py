    ...
    if cfg.TRAIN.RESUME:
        ...
        if os.path.exists(resume):
            if os.path.isdir(resume):
                # Experiment directory — pick the most recently modified last*.ckpt
                ckpt_dir = pjoin(resume, "checkpoints")
                last_ckpts = sorted(
                    [f for f in os.listdir(ckpt_dir) if f == "last.ckpt" or f.startswith("last-v")],
                    key=lambda f: os.path.getmtime(pjoin(ckpt_dir, f)),
                )
                if not last_ckpts:
                    raise ValueError(f"No last*.ckpt found in {ckpt_dir}")
                cfg.TRAIN.RESUME = pjoin(ckpt_dir, last_ckpts[-1])

                # Wandb
                wandb_latest_dir = pjoin(resume, "wandb", "latest-run")
                if os.path.isdir(wandb_latest_dir):
                    wandb_files = os.listdir(wandb_latest_dir)
                    wandb_run = [item for item in wandb_files if "run-" in item][0]
                    cfg.LOGGER.WANDB.params.id = wandb_run.replace("run-", "").replace(".wandb", "")
            else:
                # Checkpoint file path provided directly
                cfg.TRAIN.RESUME = resume

            # A real resume should restore optimizer/scheduler/trainer state
            # from the checkpoint, not reload model weights as pretrained.
