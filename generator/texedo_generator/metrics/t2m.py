    ...
    def _get_t2m_evaluator(self, cfg):
        ...
        t2m_checkpoint = torch.load(os.path.join(
            cfg.METRIC.TM2T.t2m_path, dataname, "t2m/text_mot_match/model/finest.tar"),
                                    map_location="cpu")
        print("✓ Load T2M pretrained from {}".format(os.path.join(
            cfg.METRIC.TM2T.t2m_path, dataname, "t2m/text_mot_match/model/finest.tar")))
        ...
        if self.text:
            ...
            if count_seq > self.R_size:
                ...
                for i in range(count_seq // self.R_size):
                    ...
                    # [bs=32, 32]
                    # BF16 FIX: .trace() has no bf16 kernel; .float() casts to fp32.
                    dist_mat = euclidean_distance_matrix(
                        group_texts, group_motions).nan_to_num().float()
                    # print(dist_mat[:5])
                ...
                print("warning: count_seq < self.R_size")
                metrics["Matching_score"] = 0.0
                for k in range(self.top_k):
                    metrics[f"R_precision_top_{str(k+1)}"] = 0.0
            ...
            if count_seq > self.R_size:
                ...
                print("warning: count_seq < self.R_size")
                metrics["gt_Matching_score"] = 0.0
                for k in range(self.top_k):
                    metrics[f"gt_R_precision_top_{str(k+1)}"] = 0.0

        # ---------------------------------------------------------------
        # BF16 FIX: .numpy() does not support BFloat16 tensors.
        # When training with precision='bf16-mixed', embeddings may be bf16.
        # Cast to float32 before converting to numpy.
        # If you switch to fp32 training this .float() is a harmless no-op.
        # ---------------------------------------------------------------
